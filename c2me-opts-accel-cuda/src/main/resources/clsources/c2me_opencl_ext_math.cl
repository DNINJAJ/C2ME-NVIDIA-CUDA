...
kernel void aquifer_data_prefill(global const void * restrict const const_data, global const void * restrict const rw_data,
                                 const int32_t logicalX, const int32_t logicalY, const int32_t logicalZ) {
    if (get_global_id(0) >= logicalX || get_global_id(1) >= logicalZ || get_global_id(2) >= logicalY) return;
