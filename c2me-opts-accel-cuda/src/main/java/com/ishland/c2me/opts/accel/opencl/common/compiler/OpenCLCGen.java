...
//                            .append("    ").append(name).append("(const_data, rw_data, extra_out);\n");
            ...
            if (!interpolatorPrefills.isEmpty()) {
                this.pendingSource
                        ...
                        .append("kernel void df_interpolator_buffer_prefill_kernel(global const void * restrict const const_data, global void * restrict const rw_data, const int32_t logicalX, const int32_t logicalY, const int32_t logicalZ) {\n")
                        .append("    if (get_global_id(0) >= logicalX || get_global_id(1) >= logicalY || get_global_id(2) >= logicalZ) return;\n")
                        .append("    ").append("if (!const_data || !rw_data) {\n")
                        .append("    ").append("    ").append("#ifdef DEBUG\n")
                        .append("    ").append("    ").append("printf(\"trap: !const_data || !rw_data\\n const_data=%p rw_data=%p\\n\", const_data, rw_data);\n")
                        .append("    ").append("    ").append("#endif\n")
